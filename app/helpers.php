...
function getAllFileDirectory($path)
{
    $files = [];
...
function getConnection($database_id = 0, $database_name = 0)
{
    if ($database_id == 0) {
    ...
    if ($database_id) {
        $params = Database::where('user_id', getUserId())->find($database_id);


    }
    if ($database_name) {
        $params = Database::where('name', $database_name)
            ->first();


    }
    if ($params) {
        config(['database.connections.' . $params->name => [
            'driver' => 'mysql',
...
function makeException($exception, $file, $function, $line, $delay = 3, $error_message = "خطا در ثبت اطلاعات ")
{
    dispatch(new ExceptionHandler($exception->getMessage(), $file, $function, $line))->delay(Carbon::now()->addSeconds($delay));
    return back()->with('error', $error_message);
}

function web_response($request, $view, $data = [], $extraData = [])
{
    ...
    if ($request->ajax()) {
        ...
        return view($view . '.table', compact('data', 'extraData', 'count'))->render();

    }
    return view($view . '.list', compact('data', 'extraData', 'count'));
...
function getUsername($guard = 'user')
{
...
function app_response($status, $message, $data = [], $error = [], $detail = "")

{

    return response()->json(['status' => $status, 'message' => $message, 'data' => $data, 'error' => $error, 'detail' => $detail]);
}
