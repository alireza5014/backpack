...
namespace App\Http\Controllers\All\alireza\alireza_ttt;

use App\Model\All\alireza\alireza_ttt\User;
   ...
   public $username;
    ...
    public function create(Request $request)
    {
        ...
        try {
            $data = User::create(
                [
                    "id"=> $request->id,"fname"=> $request->fname,"lname"=> $request->lname,"email"=> $request->email,"mobile"=> $request->mobile,"password"=> $request->password,"address"=> $request->address,"created_at"=> $request->created_at,"updated_at"=> $request->updated_at
                ]
            );
        } catch (Exception $exception) {
            $code = 400;
